...
class SettingsDialog:
    ...
    def _create_widgets(self) -> None:
        ...
        ttk.Spinbox(thumb_frame, from_=200, to=1600, increment=100, textvariable=self.thumbnail_size_var, width=8).pack(
            side=RIGHT
        )
        ...
        ttk.Spinbox(quality_frame, from_=30, to=100, increment=5, textvariable=self.jpeg_quality_var, width=8).pack(
            side=RIGHT
        )
        ...
        ttk.Spinbox(arrow_len_frame, from_=10, to=100, increment=5, textvariable=self.arrow_length_var, width=8).pack(
            side=RIGHT
        )
        ...
        ttk.Spinbox(arrow_width_frame, from_=1, to=10, increment=1, textvariable=self.arrow_width_var, width=8).pack(
            side=RIGHT
        )
        ...
        ttk.Button(btn_frame, text="Guardar", bootstyle="success", command=self._save).pack(side=RIGHT, padx=5)
        ttk.Button(btn_frame, text="Cancelar", bootstyle="secondary", command=self.dialog.destroy).pack(side=RIGHT)
        ttk.Button(btn_frame, text="Restablecer", bootstyle="warning-outline", command=self._reset_defaults).pack(
            side=LEFT
        )
