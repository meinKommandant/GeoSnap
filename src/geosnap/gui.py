...
class GeoPhotoApp:
    def __init__(self, root):
        ...
        self.mode_text_var = tk.StringVar(value="Modo: FOTOS ➔ KMZ + EXCEL") 
        ...
        self.status_label = ttk.Label(self.progress_frame, text="Esperando...", anchor="w")
    ...
    def _crear_selector_carpeta(self, parent, row, label_text, var):
    ...
    def _update_ui_elements(self, percentage, message):
    ...
    def start_generation_thread(self):
        ...
        thread = threading.Thread(
            ...
            args=(input_path, output_path, project_name, reverse_mode, excel_path)
        )
        thread.daemon = True
        thread.start()
    ...
    def cancel_process(self):
        if messagebox.askyesno("Cancelar", "¿Detener proceso?"):
            self.stop_event.set()
            self.status_label.config(text="Cancelando...", bootstyle="danger")
            self.btn_cancel.config(state=DISABLED)

    def _run_backend_process(self, input_path, output_path, project_name, reverse_mode=False, excel_path=""):
        try:
            if reverse_mode:
    ...
    def _reset_ui_state(self):
        ...
        self.btn_generate.config(state=NORMAL, text="GO")
        self.btn_cancel.config(state=DISABLED)
        self.status_label.config(bootstyle="default")

    def _show_success(self, message):
        self._reset_ui_state()
        self.status_label.config(text="✅ Listo.", bootstyle="success")
        self.progress_var.set(100)
        messagebox.showinfo("Éxito", message)

    def _show_cancelled(self):
        self._reset_ui_state()
        self.status_label.config(text="⛔ Cancelado.", bootstyle="danger")
        self.progress_var.set(0)
        messagebox.showinfo("Info", "Proceso cancelado.")

    def _show_warning(self, message):
        self._reset_ui_state()
        self.status_label.config(text="⚠️ Alerta.", bootstyle="warning")
        self.progress_var.set(0)
    ...
    def _toggle_mode_ui(self):
        ...
        if reverse:
            ...
            self.mode_text_var.set("Modo: EXCEL ➔ KMZ")
            self.status_label.config(text="Modo Reconstrucción: Selecciona Excel y Fotos.")
        else:
            self.excel_label.grid_remove()
            self.excel_entry.grid_remove()
            self.excel_btn.grid_remove()
            ...
            self.mode_text_var.set("Modo: FOTOS ➔ KMZ + EXCEL")
            self.status_label.config(text="Listo.")
        
        # SIEMPRE "GO"
        self.btn_generate.config(text="GO")
